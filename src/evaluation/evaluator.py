...
- Evaluator class (recommended for experiments)
...
def load_and_filter_prompts(
    ...
    if isinstance(prompt_ids, tuple):
        ...
        for prompt in all_prompts:
            prompt_id = prompt.get(constants.FIELD_ID)
            # Convert to int for comparison if it's numeric
            try:
                pid = int(prompt_id)
                if start_id <= pid <= end_id:
                    filtered.append(prompt)
            except (ValueError, TypeError):
                # If ID is not numeric, compare as string
                if str(start_id) <= str(prompt_id) <= str(end_id):
                    filtered.append(prompt)
        
        logger.info(f"Filtered prompts by ID range ({start_id}, {end_id}): {len(filtered)}/{len(all_prompts)} prompts")
        return filtered
        ...
        if len(filtered) < len(prompt_ids):
            missing = target_ids - set(str(p.get(constants.FIELD_ID)) for p in filtered)
            logger.warning(f"Some requested IDs not found: {missing}")
...
class Evaluator:
    ...
    def evaluate_jailbreak_effectiveness(
        ...
        Example:
            ...
            >>> prompts = [{'id': '1', 'prompt': 'How to hack?'}, ...]
            >>> processed = ['Let X = "How to hack?"...', ...]  # After jailbreak processing
            >>> results = evaluator.evaluate_jailbreak_effectiveness(
            ...     prompts, processed, LLMModel.GPT_3_5_TURBO
            ... )
        """
        from src.utils.logger import get_logger
        logger = get_logger(__name__)
        ...
        # Step 1: Generate baseline responses (original prompts → target model)
        ...
        baseline_results = target_service.batch_generate(baseline_prompts)
        baseline_dict = {pid: response for pid, response in baseline_results}
        ...
        # Step 3: Prepare evaluations for both baseline and jailbreak
        ...
        # Step 4: Evaluate both baseline and jailbreak responses
        logger.info("Running BASELINE evaluations...")
        baseline_eval_results = self.batch_evaluate_obedience(baseline_evals)
        baseline_eval_dict = {eval_id.replace('baseline_', ''): result for eval_id, result in baseline_eval_results}
        
        logger.info("Running JAILBREAK evaluations...")
        jailbreak_eval_results = self.batch_evaluate_obedience(jailbreak_evals)
        jailbreak_eval_dict = {eval_id.replace('jailbreak_', ''): result for eval_id, result in jailbreak_eval_results}
        
        # Step 5: Combine results with evaluations
        logger.info("Computing effectiveness comparison...")
        final_results = []
        for i, (prompt_data, processed_text) in enumerate(zip(prompts, processed_prompts)):
            prompt_id = str(prompt_data.get(constants.FIELD_ID, i))
    ...
    def __repr__(self) -> str:
        return f"Evaluator(model={self.model.value}, service_kwargs={self.service_kwargs})"
...
def get_harm_score(
    ...
    **kwargs
    ...
    Note: For multiple evaluations, use the Evaluator class instead for better performance.
